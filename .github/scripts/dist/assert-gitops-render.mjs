...
const EXPECTED_GATEWAY_ENV = new Map([
...
function splitRenderedDocs(rendered) {
    return rendered
        .split(/^---\s*$/m)
        .map((raw) => raw.trim())
        .filter(Boolean)
...
function extractGatewayEnv(raw) {
    const env = new Map();
...
async function loadRendered(options) {
...
function assertFrontendRoute(docs) {
...
function assertFrontendService(docs) {
...
function assertGatewayEnv(docs) {
...
function assertDeploymentServicePortPairs(docs) {
