...
test('Portfolio Stack uses Terraform-managed S3 Vectors for the Bedrock Knowledge Base', async () => {
  ...
  const dataSourceStart = main.indexOf('resource "aws_bedrockagent_data_source" "guide"');
  assert.notEqual(dataSourceStart, -1, 'Bedrock data source should be declared');
  const nextResourceStart = main.indexOf('\nresource "', dataSourceStart + 1);
  const dataSource = main.slice(dataSourceStart, nextResourceStart === -1 ? undefined : nextResourceStart);
  assert.match(dataSource, /vector_ingestion_configuration[\s\S]*chunking_strategy\s+=\s+"NONE"/, 'pre-chunked curated docs should disable Bedrock re-chunking');
  assert.doesNotMatch(dataSource, /fixed_size_chunking_configuration|chunking_strategy\s+=\s+"FIXED_SIZE"|max_tokens\s+=\s+20|overlap_percentage\s+=\s+1/, 'Portfolio data source should not retain fixed-size Bedrock chunking');
