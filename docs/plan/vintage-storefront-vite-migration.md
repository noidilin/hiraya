...
- Deployed reset rollout hardening is implemented: the GitOps restore dump now includes Hiraya Furugi product and product image rows for the active products database, keeps legacy dump foreign-key rows valid, seeds active auth/orders demo QA rows after restore, and has script-level regression coverage plus a deployed route/mutating QA checklist in the reset runbook.
